...
const price = computed(() => {
  if (scalePlan.planType === ScalePlanTypeEnum.enum.ENTRY) {
    return scalePlan.limitPrice
  }
  return scalePlan.targetPrice ?? 0
})
...
const classes = computed(() => {
  ...
  return 'bg-slate-900/60 border-none'
})
</script>
    ...
    <slot name="header">
      <CardHeader class="flex items-center">
        <CardTitle class="text-base text-slate-100"
          >Execute scale plan {{ scalePlan.label }}</CardTitle
        >
      </CardHeader>
    </slot>
    <CardContent :class="extraClass ? 'p-0' : ''">
      <form
